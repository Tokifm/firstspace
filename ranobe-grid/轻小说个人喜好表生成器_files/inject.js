...
#readProInner_box {
	...
	toolbar.innerHTML = `
	<div id="readProInner_box"><div id="readProInner"></div></div>
	<details>
		<summary><span class="btn icons_applogo" title="影深"></span></summary>
	    <div class="btn_top">
			<div class='text_loading'>关于 影深<br />版本: 0.0.5</div>
		</div>
	</details>
					<details>
			...
			if (b.length != 0) {
				...
				if (b.length != 0) {
					if (a < 20 && b.length < 50 && b.search("^.*书名：.*$") == 0 || a < 4 && b.length < 50 && b.search("^.*《.*》.*$") == 0) {
						...
						if (b.length < 40 && b.search("^# .*卷 .*$") == 0 ||b.length < 50 &&  b.search("^☆、第.*卷$") == 0 ||b.length < 40 &&  b.search("^第.*卷.*$") == 0 ||b.length < 40 &&  b.search("^第.*部.*$") == 0) {
						...
						} else if (b.length < 20 && b.search("^([0-9]|[0-9][0-9]|[0-9][0-9][0-9]|[0-9][0-9][0-9][0-9]) .*$") == 0 || b.length < 40 && b.search("^☆、第.*节") == 0 ||b.length < 30 &&  b.search("^第.*节.*$") == 0 ||b.length < 20 && b.search("^第.*页.*$") == 0) {
	...
	var readProInner = document.getElementById("readProInner");
	var readProInnertext = document.getElementById("readProInnertext");
	document.addEventListener('scroll', function(e) {
		var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
		var scrollHeight = document.documentElement.scrollHeight;
		var clientHeight = document.documentElement.clientHeight;
		if(scrollHeight > 2000) {
		readProInner.style.width = ( scrollTop / (scrollHeight-clientHeight) ) * 100 + '%';
} else {
		readProInner.style.width = '0px';
}
	})
